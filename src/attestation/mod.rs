...
pub async fn create_quote(
    ...
    let input_data =
        QuoteInputData::new(account_id.clone(), *public_key.as_bytes(), nonce, context);
...
pub async fn create_quote(
    ...
    let input_data = QuoteInputData::new(account_id, *public_key.as_bytes(), nonce, context);
...
#[cfg(feature = "production")]
pub fn get_pck(_account_id: AccountId32) -> Result<BoundedVecEncodedVerifyingKey, AttestationErr> {
    let quote_raw = configfs_tsm::create_quote([0; 64])
        .map_err(|e| AttestationErr::QuoteGeneration(format!("{:?}", e)))?;
    let quote = tdx_quote::Quote::from_bytes(&quote_raw)?;
...
#[cfg(not(feature = "production"))]
pub fn get_pck(account_id: AccountId32) -> Result<BoundedVecEncodedVerifyingKey, AttestationErr> {
    use rand::{rngs::StdRng, SeedableRng};
